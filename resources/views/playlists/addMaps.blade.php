...
@section('content')
    <div class="title-container p-20">
        <h1 class="title round-20">add maps to playlist</h1>
    </div>
    ...
    <form method="POST" action="{{ route('playlists.updateMaps', $playlist->id) }}">
                ...
                <div class="map-card-compact round-20 flex p-10"
                    ...
                    <div class="map-right-container g-10 flex">
                        <div class="map-settings-compact g-10 flex flex-col">
                            <div class="map-settings-card-compact flex flex-f-center p-5 g-3 round-10 medium-card" id="cs">
                                <span>CS</span>
                                <span>{{ trim_float($map->cs) }}</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center p-5 g-3 round-10 medium-card" id="hp">
                                <span>HP</span>
                                <span>{{ trim_float($map->hp) }}</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center p-5 g-3 round-10 medium-card" id="ar">
                                <span>AR</span>
                                <span>{{ trim_float($map->ar) }}</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center p-5 g-3 round-10 medium-card" id="od">
                                <span>OD</span>
                                <span>{{ trim_float($map->od) }}</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center p-5 round-10 medium-card" id="sr">
                                <span>{{ number_format(ceil($map->sr * 100) / 100, 2) }}★</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center p-5 round-10 medium-card" id="length">
                                <span>{{ gmdate('i:s', $map->length) }}</span>
                            </div>
                            <div class="map-settings-card-compact flex flex-f-center g-5 round-10 large-card" id="favorite-compact">
                        ...
                        <div class="map-actions flex g-10">
                            <a class="map-actions-card-compact flex flex-f-center p-5 g-5 round-10 view no-link" href="{{ route('maps.details', $map->id) }}">
                            ...
                            <a class="map-actions-card-compact flex flex-f-center p-5 g-5 round-10 link-osu no-link" href="{{ "https://osu.ppy.sh/beatmapsets/" . $map->setId . "#osu/" . $map->mapId }}" target="_blank">
        ...
        <br>
        <button type="submit" class="button round-20 return m-auto p-10 bold flex flex-f-center g-5">
